...
impl<'pass, 'doc> QueryTrailCodeGenPass<'pass, 'doc> {
    fn gen_query_trail(&mut self) {
        self.tokens.extend(quote! {
            use juniper_from_schema::{Walked, NotWalked, QueryTrail, DowncastQueryTrail};
        })
    }

    fn gen_from_default_scalar_value(&mut self) {
        self.tokens.extend(quote ! {
           use juniper_from_schema::FromDefaultScalarValue;
        });
        ...
        let gen_impl = |to: &str, variant: &str| {
            ...
            quote! {
                impl<'a, 'b> FromDefaultScalarValue<#to> for &'a &'b DefaultScalarValue {
                    fn from(self) -> #to {
                        let warn = |kind| {
                        ...
                        match self {
                            DefaultScalarValue::#variant(x) => x.to_owned(),
                            other => match other {
    ...
    fn gen_from_look_ahead_value(&mut self) {
        self.tokens.extend(quote! {
           use juniper_from_schema::FromLookAheadValue;
        });

        let gen_scalar_impl = |to: &str| {
            let to = format_ident!("{}", to);
            quote! {
                impl<'a, 'b> FromLookAheadValue<#to> for &'a LookAheadValue<'b, DefaultScalarValue>
                {
                    fn from(self) -> #to {
                        let warn = |kind| {
                            panic!("Failed converting look ahead value. Expected scalar type got `{kind}`")
                        };
                        match self {
        ...
        if self.pass.ast_data.url_scalar_defined() {
            self.tokens.extend(quote! {
                impl<'a, 'b> FromLookAheadValue<url::Url> for &'a LookAheadValue<'b, DefaultScalarValue>
                {
        ...
        if self.pass.ast_data.uuid_scalar_defined() {
            self.tokens.extend(quote! {
                impl<'a, 'b> FromLookAheadValue<uuid::Uuid> for &'a LookAheadValue<'b, DefaultScalarValue>
                {
        ...
        if self.pass.ast_data.date_scalar_defined() {
            self.tokens.extend(quote! {
                impl<'a, 'b> FromLookAheadValue<chrono::NaiveDate> for &'a LookAheadValue<'b, DefaultScalarValue>
                {
                    fn from(self) -> chrono::NaiveDate {
                        let s = FromLookAheadValue::<String>::from(self);
                        match chrono::NaiveDate::parse_from_str(&s, "%Y-%m-%d") {
                            Ok(date) => date,
                            Err(e) => panic!("Error parsing NaiveDate. Format used is `%Y-%m-%d`\n{}", e),
                        }
                    }
                }
            });
        ...
        if self.pass.ast_data.date_time_scalar_defined() {
            self.tokens.extend(quote! {
                impl<'a, 'b> FromLookAheadValue<chrono::DateTime<chrono::Utc>>
                    for &'a LookAheadValue<'b, DefaultScalarValue>
                {
                ...
                impl<'a, 'b> FromLookAheadValue<chrono::NaiveDateTime> for &'a LookAheadValue<'b, DefaultScalarValue>
                {
